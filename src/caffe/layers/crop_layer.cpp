...
void CropLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  offsets_ = vector<int>(input_dim, 0);
  ...
  for (int i = 0; i < input_dim; ++i) {
    ...
    if (i >= start_axis) {
	  ...
	  if (param.center_align()) {
        crop_offset = (bottom[0]->shape(i) - bottom[1]->shape(i)) / 2;
	  } 
	  else {
        if (param.offset_size() == 1) {
...
void CropLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  crop_copy(bottom, top, offsets_, indices, 0, bottom_data, top_data, true);
}
...
void CropLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    std::vector<int> indices(top[0]->num_axes(), 0);
    crop_copy(bottom, top, offsets_, indices, 0, top_diff, bottom_diff, false);
  }
}
