...
void WeightedSoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  method_ = this->layer_param_.weighted_softmax_loss_param().method();
  alpha_ = this->layer_param_.weighted_softmax_loss_param().alpha();
  gamma_ = this->layer_param_.weighted_softmax_loss_param().gamma();
...
void WeightedSoftmaxWithLossLayer<Dtype>::Reshape(
  ...
  channel_num_ = bottom[0]->count() / outer_num_ / inner_num_;
  std::vector<int> weights_shape(2);
  weights_shape[0] = outer_num_;
  weights_shape[1] = channel_num_;
  channel_weights_.Reshape(weights_shape);
  caffe_memset(outer_num_ * channel_num_, 0, channel_weights_.mutable_cpu_data());
...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  Dtype* weights_data = channel_weights_.mutable_cpu_data();
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      DCHECK_LT(label_value, channel_num_);
	  weights_data[i * channel_num_ + label_value] += 1;
    }
	Dtype max_weight = 0;
	for (int j = 0; j < channel_num_; ++j) {
	  if (weights_data[i * channel_num_ + j] > max_weight) {
		max_weight = weights_data[i * channel_num_ + j];
	  }
	}
	for (int j = 0; j < channel_num_; ++j) {
	  if (weights_data[i * channel_num_ + j] > 0)
	    weights_data[i * channel_num_ + j] = max_weight / weights_data[i * channel_num_ + j];
	}
  }
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
	  ...
	  loss -= weights_data[i * channel_num_ + label_value] * 
...
void WeightedSoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* label = bottom[1]->cpu_data();
	const Dtype* weights_data = channel_weights_.cpu_data();
	int dim = prob_.count() / outer_num_;
    int count = 0;
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
		  ...
		  for (int k = 0; k < bottom[0]->shape(softmax_axis_); ++k) {
			bottom_diff[i * dim + k * inner_num_ + j] *= 
				weights_data[i * channel_num_ + label_value];
		  }
          ++count;
        }
      }
