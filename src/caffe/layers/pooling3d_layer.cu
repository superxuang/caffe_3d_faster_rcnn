...
void Pooling3DLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    MaxPoolForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        kernel_l_, kernel_h_, kernel_w_, stride_l_, stride_h_, stride_w_, 
        pad_l_, pad_h_, pad_w_, top_data,
        mask, top_mask);
    break;
...
void Pooling3DLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  case PoolingParameter_PoolMethod_STOCHASTIC:
    ...
    StoPoolBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        top[0]->num(), channels_, length_, height_, width_, 
        pooled_length_, pooled_height_, pooled_width_, kernel_l_, kernel_h_, kernel_w_, 
        stride_l_, stride_h_, stride_w_, bottom_diff);
    break;
  default:
    LOG(FATAL) << "Unknown pooling method.";
  }
