...
#include "caffe/layers/mhd_data_with_dense_layer.hpp"
...
MHDDataWithDenseLayer<Dtype>::~MHDDataWithDenseLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void MHDDataWithDenseLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  itk::ObjectFactoryBase::RegisterFactory(itk::MetaImageIOFactory::New());
  typedef itk::ImageFileReader<ImageType> ImageReaderType;
  typedef itk::ImageFileReader<LabelType> LabelReaderType;

  const MHDDataWithDenseParameter& mhd_data_param = this->layer_param_.mhd_data_with_dense_param();
  ...
  bool output_data_in_dense = mhd_data_param.output_data_in_dense();
  vector<int> dense_shape(5);
  dense_shape[0] = batch_size;
  dense_shape[1] = contour_win_width_list.win_width_size() + output_data_in_dense;
  dense_shape[2] = mhd_data_param.resample_volume_length();
  dense_shape[3] = mhd_data_param.resample_volume_height();
  dense_shape[4] = mhd_data_param.resample_volume_width();

  for (int i = 0; i < PREFETCH_COUNT; ++i) {
    this->prefetch_[i].data_.Reshape(data_shape);
    this->prefetch_[i].label_.Reshape(label_shape);
    this->prefetch_[i].dense_.Reshape(dense_shape);
  }

  top[0]->Reshape(data_shape);
  top[1]->Reshape(label_shape);
  top[2]->Reshape(dense_shape);
...
template <typename Dtype>
void MHDDataWithDenseLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void MHDDataWithDenseLayer<Dtype>::load_batch(ExBatch<Dtype>* batch) {
  typedef itk::ImageFileReader<ImageType> ImageReaderType;
  typedef itk::ImageFileReader<LabelType> LabelReaderType;
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (item_id == 0) {
      ...
      dense_shape[0] = batch_size;
      dense_shape[1] = contour_win_width_list.win_width_size() + output_data_in_dense;
      dense_shape[2] = size_src[2];
      dense_shape[3] = size_src[1];
      dense_shape[4] = size_src[0];
      batch->dense_.Reshape(dense_shape);
    }
    ...
    offset_inds[3] = 0;
    offset_inds[4] = 0;
    Dtype* image_data = batch->data_.mutable_cpu_data() + batch->data_.offset(offset_inds);
    Dtype* label_data = batch->label_.mutable_cpu_data() + batch->label_.offset(offset_inds);
    ...
    for (int chn = 0; chn < contour_win_width_list.win_width_size(); ++chn) {
      offset_inds[1] = chn + output_data_in_dense;
      Dtype* dense_data = batch->dense_.mutable_cpu_data() + batch->dense_.offset(offset_inds);
      float win_level = contour_win_level_list.win_level(chn);
      float win_width = contour_win_width_list.win_width(chn);
      for (int img_index = 0; img_index < src_buffer_length; ++img_index) {
        dense_data[img_index] = exp(-(image_output_buffer[img_index] - win_level) * (image_output_buffer[img_index] - win_level) / (win_width * win_width));
      }
    }
    ...
    if (output_data_in_dense) {
      offset_inds[1] = 0;
      Dtype* dense_data = batch->dense_.mutable_cpu_data() + batch->dense_.offset(offset_inds);
      for (int img_index = 0; img_index < src_buffer_length; ++img_index) {
        dense_data[img_index] = image_data[img_index];
      }
    }
    ...
    //    memcpy(output_prob->GetBufferPointer(), batch->dense_.mutable_cpu_data(), sizeof(Dtype) * pixel_num_1);
    //    ImageWriterType::Pointer writer_image = ImageWriterType::New();
    ...
    //}
    //{
    //    memcpy(output_image->GetBufferPointer(), batch->dense_.mutable_cpu_data() + pixel_num_1, sizeof(Dtype) * pixel_num_1);
    //    ImageWriterType::Pointer writer_image = ImageWriterType::New();
    //    writer_image->SetInput(output_image);
...
INSTANTIATE_CLASS(MHDDataWithDenseLayer);
REGISTER_LAYER_CLASS(MHDDataWithDense);
