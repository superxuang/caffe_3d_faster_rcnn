...
template <typename Dtype>
  __global__ void UpsampleForward(const int nthreads, 
	  int in_w, int in_h, int in_l, 
      int out_w, int out_h, int out_l, 
	  const Dtype* bottom_data, const Dtype* bottom_mask, 
	  Dtype* top_data) {
    CUDA_KERNEL_LOOP(index, nthreads) {
      int offset = index / (in_w * in_h * in_l) * out_w * out_h * out_l;
...
void UpsampleLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  UpsampleForward<Dtype><<<CAFFE_GET_BLOCKS(bottom_count), CAFFE_CUDA_NUM_THREADS>>>(
	  bottom_count, 
	  bottom[0]->shape(4), bottom[0]->shape(3), bottom[0]->shape(2),
	  top[0]->shape(4), top[0]->shape(3), top[0]->shape(2), 
	  bottom_data, bottom_mask, top_data);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
  __global__ void UpsampleBackward(const int nthreads, 
	  int in_w, int in_h, int in_l,
      int out_w, int out_h, int out_l, 
	  const Dtype* top_diff, const Dtype* bottom_mask, 
	  Dtype* bottom_diff) {
    CUDA_KERNEL_LOOP(index, nthreads) {
      int offset = index / (in_w * in_h * in_l) * out_w * out_h * out_l;
      int upsample_idx = static_cast<int>(bottom_mask[index]);
...
void UpsampleLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    UpsampleBackward<Dtype><<<CAFFE_GET_BLOCKS(bottom_count), CAFFE_CUDA_NUM_THREADS>>>(
		...
		bottom[0]->shape(4), bottom[0]->shape(3), bottom[0]->shape(2), 
		top[0]->shape(4), top[0]->shape(3), top[0]->shape(2), 
		top_diff, bottom_mask, bottom_diff);
    CUDA_POST_KERNEL_CHECK;
  }
}
