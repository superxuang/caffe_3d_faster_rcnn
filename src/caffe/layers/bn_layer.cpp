...
namespace caffe {
  ...
  void BNLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    if (top.size() > 1) {
        ...
        top[1]->Reshape(1, C_, 1, 1, 1);
    }
    if (top.size() > 2) {
        // top blob for batch variance
        top[2]->Reshape(1, C_, 1, 1, 1);
    }
    ...
    // mean
    spatial_mean_.Reshape(N_, C_, 1, 1, 1);
    batch_mean_.Reshape(1, C_, 1, 1, 1);
    // variance
    spatial_variance_.Reshape(N_, C_, 1, 1, 1);
    batch_variance_.Reshape(1, C_, 1, 1, 1);
    // buffer blob
    buffer_blob_.Reshape(N_, C_, L_, H_, W_);

    // fill spatial multiplier
    spatial_sum_multiplier_.Reshape(1, 1, L_, H_, W_);
    ...
    batch_sum_multiplier_.Reshape(N_, 1, 1, 1, 1);
  ...
  void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    if (this->blobs_.size() > 0) {
      ...
      this->blobs_[0].reset(new Blob<Dtype>(1, C_, 1, 1, 1));
      ...
      this->blobs_[1].reset(new Blob<Dtype>(1, C_, 1, 1, 1));
  ...
  void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    case BNParameter_BNMode_LEARN:
      ...
      // EX across spatial
      caffe_cpu_gemv<Dtype>(CblasNoTrans, N_ * C_, L_ * H_ * W_,
          Dtype(1. / (L_ * H_ * W_)), bottom_data,
      ...
      caffe_cpu_gemv<Dtype>(CblasNoTrans, N_ * C_, L_ * H_ * W_,
          Dtype(1. / (L_ * H_ * W_)), buffer_blob_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Dtype(0),
  ...
  void BNLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    case BNParameter_BNMode_LEARN:
      ...
      caffe_cpu_gemv<Dtype>(CblasNoTrans, N_ * C_,
          L_ * H_ * W_, Dtype(1), buffer_blob_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Dtype(0),
      ...
      caffe_cpu_axpby(buffer_blob_.count(), Dtype(1),
          buffer_blob_.cpu_data(), Dtype(-1. / (N_ * L_ * H_ * W_)),
          bottom_diff);
