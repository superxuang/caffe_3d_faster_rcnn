...
__global__ void ROIPoolForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int roi_start_w = round(bottom_rois[1] * spatial_scale_xy);
	int roi_start_h = round(bottom_rois[2] * spatial_scale_xy);
    int roi_start_l = round(bottom_rois[3] * spatial_scale_z);
	int roi_end_w = round(bottom_rois[4] * spatial_scale_xy);
	int roi_end_h = round(bottom_rois[5] * spatial_scale_xy);
    int roi_end_l = round(bottom_rois[6] * spatial_scale_z);
...
template <typename Dtype>
void ROICropLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ROICropLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(ROICropLayer);
